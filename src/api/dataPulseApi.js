...
export async function payDirectViaPaystack(bundle, phone, storeId, userEmail) {
  ...
  return new Promise((resolve, reject) => {
    if (!window.PaystackPop) {
      // Development mock fallback if Paystack runtime script frame fails to build on native DOM context
      resolve({ reference: ref });
      return;
    }
    const handler = window.PaystackPop.setup({
      key:      PAYSTACK_KEY,
      // Fallback email generated smoothly from phone structure to fulfill Paystack's requirements safely
      email:    userEmail || `${phone.replace(/^0/, '')}@scifidata.com`,
      ...
      metadata: {
        ...
        custom_fields: [
          ...
          { display_name: 'Network', variable_name: 'network',     value: bundle.network || 'MTN' },
