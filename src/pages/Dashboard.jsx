...
import { fetchStoreByOwnerId, fetchOrdersByStoreId } from '../api/dataPulseApi';
        ...
        <tbody>
          {orders.map(o => (
            <tr key={o.id} className="border-b" style={{ borderColor: 'var(--border-subtle)' }}>
              <td className="py-3 text-[#F0F6FF] font-mono text-xs">{o.reference || o.id.slice(0, 8)}</td>
              <td className="py-3 text-[#94A3B8]">{o.customer_phone || o.customer_email || '—'}</td>
              <td className="py-3 text-[#F0F6FF]">GHS {Number(o.amount).toFixed(2)}</td>
